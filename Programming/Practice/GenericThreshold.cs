...
public class ThresholdMonitor<T> where T : IComparable<T>
{
    ...
    public void Update(T newValue)
    {
        ...
        bool crossed =
            newValue.CompareTo(_threshold) >= 0 &&
            _current.CompareTo(_threshold) < 0;

        if (crossed)
        {
            var EventArgument = new ThresholdChangedEventArgs<T>
            {
                OldValue = _current,
                NewValue = newValue,
            ...
            ThresholdCrossed?.Invoke(this, EventArgument);
        }
        // 2) Update _current
        _current = newValue;
    }
