namespace Practice{
    ...
    public class ShoppingCart<T> where T : Product
    {
        ...
        public void AddToCart(T product, int quantity)
        {
            ...
            //if the product with same id is there 
            //Update the Quantity!!!
            if(_cartItems.Keys.Any(key => key.Id == product.Id))
                _cartItems[product] += quantity;
                
            //Add the new entry to the dixctoary 
        ...
        public List<T> GetTopExpensiveItems(int n)
        {
            ...
            var sortedDict = _cartItems.Keys.OrderByDescending(key => key.Price).Take(n);
